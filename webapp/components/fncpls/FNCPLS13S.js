/*************************************************************************
* @ 서비스경로 : FNCPLS13S
* @ 파일명      : FNCPLS13S  금융+ > 예적금계산기 > 적금계산 > 매월모으기
* @ 화면ID	  : PBWFI010000
* @ 작성자      : 90195654
* @ 작성일      : 2025-01-13
************************** 수정이력 ****************************************
...
export default class FNCPLS13S extends BaseModule {
	...
	init () {
		console.log(' 금융+ > 예적금계산기 > 적금계산 > 매월모으기');
	...
	register() {
		...
		//tab버튼 click
		const tabs = {elements : [$$("#fncpls10s").elements[0], $$("#fncpls16s").elements[0]]};
	...
	validation() {
		...
		if(CmmUtils.removeComma(keepMoney*1) > 100000000000 ){
			...
			$$("#keepMoneyForm").find(".validate-text").text("예치금액을 10억 미만으로 입력하세요");
			return false;
		}
	...
	async search () {
		const param = {
			addMmAcldAm : CmmUtils.removeComma(this.keepMoney.value)
			, dpstmMcn : this.keepMonth.value
			, irFrcsUsgRt : $$("#keepRate").value()	
		}
		
		await GlobalApp.popupUtils.openPopup('fncpls14p', {
            showCloseBtn : true,
            params : param,
        });
