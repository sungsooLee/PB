...
export default class FNCPLS10S extends BaseModule {
	...
	register() {
		...
		const mainTabClickCallback = (event) => {
		...
		// 조회결과 tab버튼 click
		const fncpls13s = $$("#fncpls13s");
		this.registerCallback("click", fncpls13s, mainTabClickCallback);
	...
	validation() {
		...
		if(CmmUtils.removeComma(keepMoney*1) > 100000000000 ){
			...
			$$("#keepMoneyForm").find(".validate-text").text("예치금액을 1000억 미만으로 입력하세요");
			return false;
		}
		...
		if(keepMonth*1 > 600) {
			...
			$$("#keepMonthForm").find(".validate-text").text('예치기간을 600개월 이하로 입력하세요');
			return false;
		}
	...
	async search() {
		...
		const param = {
			depAm1 : CmmUtils.removeComma(this.keepMoney.value)
			, dpstmMcn1 : this.keepMonth.value
			, irFrcsUsgRt : $$("#keepRate").value()
		}
		
		await GlobalApp.popupUtils.openPopup('fncpls11p', {
            showCloseBtn : true,
            params : param,
        });
	}
