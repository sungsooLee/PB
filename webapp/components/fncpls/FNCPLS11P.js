/*************************************************************************
* @ 서비스경로 : FNCPLS11P
* @ 파일명      : FNCPLS11P  금융+ > 예적금계산기 > 예금계산 > 검색 결과
* @ 화면ID	  : PBWFI009901, PBWFI009902 (단리.복리)
* @ 작성자      : 90195654
* @ 작성일      : 2025-01-13
************************** 수정이력 ****************************************
* 날짜                    작업자                 변경내용
...
export class FNCPLS11P extends BaseModule {
	...
	init (param) {
		...
		this.setKeepMoney(param.depAm1);
		this.register();
	...
	async search(param) {
		...
		const data = await ApiUtils.sendPost('/api/fncpls/getDpidCal.json', param);
	...
	dataSet(data) {
		...
		$$('#area2').text(CmmUtils.comma(keepMoney)+"원");			// 예치금액
		...
		// 비과세(단리)
		$$("#area6").text("0.0%");										// 세율
		$$("#area7").text(CmmUtils.comma(keepMoney)+"원");			// 예치금액
		...
		$$("#area12").text(CmmUtils.comma(keepMoney)+"원");			// 예치금액
		...
		// 비과세(복리)
		$$("#area16").text("0.0%");										// 세율
		$$("#area17").text(CmmUtils.comma(keepMoney)+"원");			// 예치금액
