...
export class FND41P extends BaseModule {
	...
	init(param) {
		console.log('펀드 > 펀드상세 > 커뮤니티 > 내정보관리');
	...
	async apiCheckDupliNickname(param) {
		...
		const result = await ApiUtils.sendPost('/api/fnd/checkDupliNickname.json', param);
	...
	async apiRegistNickname(param) {
		...
		const result = await ApiUtils.sendPost('/api/fnd/updateNickname.json', param);
