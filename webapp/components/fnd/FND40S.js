...
export default class FND40S extends BaseModule {
	...
	initButton() {
		...
		const likeBtn = async (event, element) => {
			...
			if(result.resultCode === '000000') {
				...
				//좋아요 건수 조회
				const likeObj = await ApiUtils.sendPost('/api/fnd/getCmtyLikeCnt.json', param);
	...
	async registPopup(param) {
		...
		await GlobalApp.popupUtils.openPopup('fnd42p', {
            showCloseBtn : true,
            params : param,
            onCallback: (result) => {
            	if(result.data > 0) {
	...
	async MyInfoPopup(param) {
		await GlobalApp.popupUtils.openPopup('fnd41p', {
            showCloseBtn : true,
            btn2Label : '저장',
            btn2IsMain : true,
            params : {},
	...
	async apiCmtyList() {
		...
		const result = await ApiUtils.sendPost('/api/fnd/getCmtyList.json', param, true);
	...
	async apiSetCmtyLikeYn(param) {
		...
		const result = await ApiUtils.sendPost('/api/fnd/setCmtyLikeYn.json', param);
