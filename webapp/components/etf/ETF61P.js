/*************************************************************************
* @ 서비스경로 : FND61P
* @ 파일명      : FND61P ETF > 나의 관심 ETF > ETF 순서 설정
* @ 화면ID	  : PBWFU079900
* @ 작성자      : 90195654
* @ 작성일      : 2025-01-13
************************** 수정이력 ****************************************
...
export class ETF61P extends BaseModule {
    ...
    async apiCncrFndList() {
		...
		const result = await ApiUtils.sendPost('/api/etf/getCncrFndOrderList.json', {});
	...
	returnToBtn2CallBack () {
	 ...
	 async apiUpdateCncrFndSeq(param) {
			...
			const result = await ApiUtils.sendPost('/api/etf/updateCncrFndSeq.json', param);
