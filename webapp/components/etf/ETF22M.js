...
export default class ETF22M extends BaseModule {
	...
	constructor(elem) {
        ...
        [this.term, this.setTerm] = this.useState('term', '3');		//기간 default 3개월
        const riskList = [
    ...
    initButton() {
    	...
    	const termBtnClick = async () => {
    		await GlobalApp.popupUtils.openPopup('fndTem', {
                showCloseBtn : true,
                btn2Label : '조회',
                btn2IsMain : true,
                params : {
                ...
                onYes: (result) => {
                	...
                	$$("#termBtn").find(".sort").text(result.term+"개월");
                	this.callFndList();
                }
            });
    	}
    	this.registerCallback("click", termBtn,  termBtnClick);
	...
	async apiFndList() {
		...
		const result = await ApiUtils.sendPost('/api/etf/getBestFndList.json', param, true);
	...
	async apiFndListDataSet(data) {
        ...
        $.each(_data.list, function(i, item){
        	...
        	if((CmmUtils.isEmpty(item.fndEstDt) && !CmmUtils.isEmpty(item.bmmPrftRt)) || _this.calcDiff(item.fndEstDt)) {
        		 ...
        		 * 검색 시 선택한 기간에 포인트
        		 */
		        if(_this.term.value === '1' && bmmPrftRt !== '') {
		        ...
		        html += '		</div>';
		        if(_this.term.value === '3' && m3PrftRt !== '') {
		        ...
		        html += '			<dd>'+m3PrftRt+'</dd>';
		        html += '		</div>';
		        if(_this.term.value === '6' && m6PrftRt !== '') {
		        ...
		        html += '			<dd>'+m6PrftRt+'</dd>';
		        html += '		</div>';
		        if(_this.term.value === '12' && m12PrftRt !== '') {
