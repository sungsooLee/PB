/*************************************************************************
* @ 서비스경로 : ETF42P
* @ 파일명      : ETF42P ETF > ETF상세 > 커뮤니티 답글 등록 팝업
* @ 화면ID	  : PBWFU059902
* @ 작성자      : 90195654
* @ 작성일      : 2025-01-13
************************** 수정이력 ****************************************
...
export class ETF42P extends BaseModule {
	...
	constructor(elem) {
		...
		[this.fndPdcd, this.setFndPdcd] = this.useState('fndPdcd', '');					//ETF상품코드
		[this.fndNm, this.setFndNm] = this.useState('fndNm', '');						//ETF명
	...
	init (param, callbackFunc) {
		console.log('ETF > ETF상세 > 커뮤니티 > 답글 등록 : ', param );
		...
		const contentRegist = () => {
			if(cmtyTxt.value().length > 0) {
				const param = {
					...
					, fndPdcd 			: this.fndPdcd.value		//ETF상품코드         
					, fndNm 			: this.fndNm.value			//ETF명   
	...
	async apiCmtyInfo(param) {
		...
		const result = await ApiUtils.sendPost('/api/etf/getCmtyInfo.json', param);
	...
	async apiCmtyRegist(param) {
		...
		const result = await ApiUtils.sendPost('/api/etf/insertCmty.json', param);
	...
	destroy() {
		...
		this.setFndPdcd('');		//ETF상품코드
		this.setFndNm('');		//ETF상품코드
