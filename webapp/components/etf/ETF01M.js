...
export default class ETF01M extends BaseModule {
	constructor(elem, data) {
        ...
        [this.cncrList, this.setCncrList] 				= this.useState('cncrList', '');						// 관심 ETF
        [this.bestList, this.setBestList] 				= this.useState('bestList', '');						// BEST ETF
	...
	async regist() {
		...
		const tab1 = $$("#tab1").elements[0];
		const tabTargets = { elements : [tab1] };
		...
		$$('#btnMyFnd').on('click', async () => await this.moveCncrFnd());								// 나의 관심ETF 바로가기
		$$('#btnChart').on('click', async () => await this.moveChart());								// ETF/지수 비교하기 
		$$('#btnFnd').on('click', async () => await this.searchFnd());									// ETF찾기 
		$$('#btnEtfGuide').on('click', async () => await this.goEtfGuidebook());						// ETF 가이드북 
	...
	async initData() {
		const url = '/pb/api/mn/etfMain.json';
		const params = {};
		...
		if(res.resultCode === '000000') {
			...
			if(cncrLst.length > 1) {
				data = cncrLst;
				$$('#infoTxt').text('나의 관심 ETF 수익률');
				$$('#btnMyFnd').show();
			} else {
				data = bestLst;
				$$('#infoTxt').text('판매 BEST 3 ETF 수익률');
				$$('#addFndTxt').show();
			}
	...
	async loadData() {
    	...
    	let fData = res.data.dataETF.inq5Grid;
    	let fArr = [];
    ...
    */
	 ...
	 * @param 
	 */
	async goEtfGuidebook() {
		const route = 'etfGuide01m';	
		window.location.hash = route;
	}
