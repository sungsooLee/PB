...
export default class ETF21M extends BaseModule {
    ...
    constructor(elem) {
        ...
        [this.type, this.setTpye] = this.useState('type', 'SB');	//ETF목록 옵션 ( SB  : 판매, RCMD  : 이달의추천)
        const riskList = [
	...
	async apiFndListDataSet(data) {
        ...
        $.each(_data.list, function(i, item){
        	...
        	if((CmmUtils.isEmpty(item.fndEstDt) && !CmmUtils.isEmpty(item.bmmPrftRt)) || _this.calcDiff(item.fndEstDt)) {
  	        ...
  	        html += '   <dl class="sales-list">';
			html += '     <dt>월 판매액</dt>';
			html += '     <dd>'+_this.showAmtKr(Number(item.mmSelAcmAm))+'</dd>';
			html += '   </dl>';
	...
	// 금액 단위
	showAmtKr (num) {
		const result = Number(num)/100000000;
		return result.toFixed(2).replace(/\.00$/,'')+'억';
	}
