...
export default class ETF60M extends BaseModule {
    ...
    initEvent() {
    	...
    	const fndOrderPopup = async () => {
			...
			await GlobalApp.popupUtils.openPopup('etf61p', {
	            showCloseBtn : true,
	            btn2Label : '저장',
	            btn2IsMain : true,
	            params : {},
    ...
    async deleteCncrFnd(param,element) {
		...
		if(confirmRes.resultCd === '000000') {
			const result = await ApiUtils.sendPost('/api/etf/deleteCncrFnd.json', param);
    ...
    async apiCncrFndList() {
		const result = await ApiUtils.sendPost('/api/etf/getCncrFndList.json', {}, true);
    ...
    async apiCncrFndListDataSet(data) {
    	...
    	if(data.length > 0) {
	        ...
	        $.each(data, function(i, item){
		        ...
		        html += '		<button type="button" aria-label="관심ETF 삭제"  name="cncrFndDelete" value="'+item.pbCncrFndSeq+'_'+item.fndRprsCd+'">';
