...
export default class ETF30S extends BaseModule {
    ...
    async apiFndDetail() {
    	...
    	const result = await ApiUtils.sendPost('/api/etf/getFndDetail.json', param, true);
    	console.log("---result : " + JSON.stringify(result, null, 2));
    	if(result.resultCode === '000000') {
    		await this.apiFndDetailDataset(result.data);
    		
    		/**
        	 * 커뮤니티 이동,관심ETF 이벤트 추가
        	 */
    ...
    apiFndDetailDataset(data) {
    	...
    	$$("#prdKorlNm").text(data.data.prdKorlNm); //ETF명
    	...
    	$$("#prdDtlTxt").text(data.data.prdDtlTxt); //ETF설명
    	...
    	$$("#zrinLrgTpNm").text(data.data.zrinLrgTpNm); //ETF 유형
    	$$("#prdIvstRgnNm").text(data.data.prdIvstRgnNm); //투자 국가
    	$$("#fndEstDt").text(this.formatChange(data.data.fndEstDt)); //ETF 설정일
    	$$("#zrinTpNm").text(data.data.zrinTpNm); //ETF 섹터
